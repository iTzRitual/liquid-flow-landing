...
function strings(lang: Lang) {
    ...
    return {
        ...
        gitLabel: "Git:",
        conflictsIndicator: pl
            ? "⚠ Konflikty: 2 (/conflicts)"
            : "⚠ Conflicts: 2 (/conflicts)",
        disconnected: "~",
        placeholder: pl
            ? "wpisz / aby zobaczyć komendy · /exit wyjście"
            : "type / to see commands · /exit to quit",
        ...
        commands: [
            [
                "/connect",
                pl
                    ? "połącz ze sklepem (lista + dodaj nowy)"
                    : "connect to a shop (list + add new)",
            ],
            ["/templates", pl ? "wybierz szablon" : "select template"],
            [
                "/conflicts",
                pl
                    ? "rozwiąż konflikty (pojedynczo + seryjnie)"
                    : "resolve conflicts (single + bulk)",
            ],
        ...
        pickerHelp: pl
            ? "↑/↓ wybór · Enter zatwierdź · Esc wróć"
        ...
        gitItems: [
            pl
                ? "Zatwierdź wersję (checkpoint)"
                : "Checkpoint (commit version)",
...
function commitLine(lang: Lang, id: string): LogLine {
    ...
    return {
        ...
        ts: liveTs(),
        text: pl
            ? `📝 Git: zapisano wersję ${hash}`
            : `📝 Git: saved version ${hash}`,
        color: C.dim,
    };
}
...
function runningLines(lang: Lang): LogLine[] {
    ...
    const sepText =
        `── ${pl ? "Nowa sesja" : "New session"} • ${when} ` + "─".repeat(200);
    const ts = liveTs();
    return [
        {
            id: "run-sep",
        ...
        },
        {
            ...
            ts,
            text: pl
                ? `Wybrano szablon: ${s.selectedTemplate} [3]`
                : `Template selected: ${s.selectedTemplate} [3]`,
            color: C.green,
        },
        {
            id: "run-1",
            ts,
            text: pl
                ? "Folder lokalny gotowy — pliki już pobrane"
                : "Local folder ready — files already downloaded",
            color: C.green,
        },
        {
            id: "run-2",
            ts,
            text: pl
                ? "Sprawdzono niezgodności — konflikty: 0"
                : "Mismatches checked — conflicts: 0",
            color: C.green,
        },
        {
            id: "run-3",
            ts,
            text: pl
                ? `Synchronizacja aktywna — hot-reload (${s.selectedTemplate})`
                : `Synchronization active — hot-reload (${s.selectedTemplate})`,
            color: C.green,
        },
    ];
}
    ...
    return [
        {
            ...
            ts: "12:00:02",
            text: pl
                ? "Połączono ze sklepem: Ogródek"
                : "Connected to shop: Ogródek",
            color: C.green,
        },
        {
            id: "sess-1",
        ...
        },
        {
            ...
            ts: "12:00:06",
            text: pl
                ? "Pobrano 128 plików ze sklepu"
                : "Downloaded 128 files from shop",
            color: C.text,
        },
        {
            id: "sess-3",
            ts: "12:04:03",
            text: pl
                ? "Plik został zmieniony — 3/0/common/header.liquid"
                : "File changed — 3/0/common/header.liquid",
            color: C.green,
        },
        {
            id: "sess-4",
            ts: "12:04:09",
            text: pl
                ? "Plik został zmieniony — 3/0/css/layout.css"
                : "File changed — 3/0/css/layout.css",
            color: C.green,
        },
        {
            id: "sess-5",
            ts: "12:04:10",
            text: pl
                ? "📝 Git: zapisano wersję a3f21c9"
                : "📝 Git: saved version a3f21c9",
            color: C.dim,
        },
    ];
}
...
function Banner() {
    return (
        <div
            className="shrink-0 whitespace-pre leading-[1.25]"
            aria-hidden="true"
        >
            {BANNER.map((line, y) => (
                <div key={y}>
        ...
        <div className="flex shrink-0 pl-1 pt-1">
            ...
            <div className="ml-4 flex min-w-0 flex-1 flex-col justify-between whitespace-pre sm:ml-6">
                <div className="overflow-hidden">
                    <div
                        className="overflow-hidden text-ellipsis whitespace-pre font-bold"
                        style={{ color: C.title }}
                    >
            ...
            <div className="absolute inset-x-0 bottom-0 px-1">
                {lines.map((l, i) => (
                    <motion.div
                        ...
                        initial={
                            i >= animateFrom ? { opacity: 0, y: "100%" } : false
                        }
                        animate={{ opacity: 1, y: 0 }}
                        transition={{
                            duration: 0.25,
                            ease: EASE_OUT,
                            layout: shift ? undefined : { duration: 0 },
                        }}
                        className={
                            l.kind === "separator"
...
function templatesFor(lang: Lang): { label: string; locked: boolean }[] {
    ...
    return items.map(([name, version, id]) => ({
            ...
            {templates.map(({ label, locked }, i) => {
                ...
                return (
                    ...
                    >
                        ...
                        {locked && (
                            <span style={{ color: isSel ? "#0b0f14" : C.dim }}>
                                {"  "}
                                {s.lockedHint}
                            </span>
                        )}
...
function Btn({ label, active }: { label: string; active?: boolean }) {
    return (
        <span
            ...
            style={
                active
                    ? { backgroundColor: C.cyan, color: "#0b0f14" }
                    : { color: C.text }
            }
        >
            {" "}
    ...
    const card = (
        ...
        <div className="whitespace-pre">
            <div className="flex justify-between gap-2 overflow-hidden">
                <span
                    className="overflow-hidden text-ellipsis whitespace-pre"
                    style={{ color: focused ? C.cyan : C.text }}
                >
                    {focused ? "› " : "  "}
                    {name}
                </span>
    ...
    return (
        <OverlayFrame title={s.conflictsTitle}>
            {card(
                ...
                `${s.tsLocal} 07-13 14:32 · ${s.tsRemote} 07-13 12:10`,
                s.localNewer,
            )}
            <div> </div>
            {card(
                false,
                "css/theme.css",
                `${s.tsLocal} 07-12 09:15 · ${s.tsRemote} 07-13 13:58`,
                s.remoteNewer,
            )}
...
const typeSteps = (
    ...
    text: string,
    ms: number,
    key: "typed" | "formTyped" | "shellTyped",
): Step[] =>
    [...text].map((_, i) => [
        start + ms * (i + 1),
        { [key]: text.slice(0, i + 1) } as Partial<Ui>,
    ]);
...
function introScript(): [Ui, Step[]] {
    ...
    return [
        start,
        [
            ...revealSteps(440, 0, RUNNING_COUNT),
            [2650, { typed: "/" }],
            [2950, { paletteOpen: true }],
        ],
    ];
}
...
export function Terminal({
    ...
    React.useEffect(() => {
        ...
        const run = ([start, steps]: [Ui, Step[]]) => {
            setUi(start);
            const ids = steps.map(([t, p]) =>
                setTimeout(() => setUi((u) => ({ ...u, ...p })), t),
            );
    ...
    React.useEffect(() => {
        ...
        const id = setInterval(() => {
            ...
            setExtra0((prev) =>
                [...prev, fileChangedLine(lang, `e0-${n}`, n)].slice(-20),
            );
        }, 2000);
        ...
        const id = setInterval(() => {
            ...
            setExtra1((prev) =>
                [
                    ...prev,
                    n % 3 === 2
                        ? commitLine(lang, `e1-${n}`)
                        : fileChangedLine(lang, `e1-${n}`, n),
                ].slice(-20),
            );
        }, 2500);
        return () => clearInterval(id);
    ...
    React.useEffect(() => {
        ...
        if (prev.overlay === ui.overlay && prev.paletteOpen === ui.paletteOpen)
            return;
        setMorphing(true);
